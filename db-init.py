...
CREATE TABLE IF NOT EXISTS mlb_transactions (
    id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    mlb_transaction_id TEXT NOT NULL UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_mlb_transactions_player
    ON mlb_transactions(player_mlb_id);
...
BEGIN
  ...
  VALUES (
    ...
    'alumni',
    'INSERT',
    json_build_object('id', NEW.id, 'player_id', NEW.player_id, 'team_id', NEW.team_id, 'deactivated_at', NEW.deactivated_at),
    (SELECT row_hash FROM audit ORDER BY id DESC LIMIT 1),
    encode(gen_random_bytes(16), 'hex')
  );
  RETURN NEW;
