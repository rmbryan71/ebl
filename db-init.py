...
CREATE TABLE IF NOT EXISTS teams (
    ...
    name TEXT NOT NULL,
    has_empty_roster_spot INTEGER NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS mlb_roster_changes (
    id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    player_mlb_id INTEGER NOT NULL REFERENCES players(mlb_id),
    change_date DATE NOT NULL,
    change_type TEXT NOT NULL CHECK (change_type IN ('add', 'remove')),
    source TEXT,
    notes TEXT,
    UNIQUE (player_mlb_id, change_date, change_type)
);
...
CREATE TABLE IF NOT EXISTS roster_move_requests (
    ...
    status TEXT NOT NULL DEFAULT 'pending'
        CHECK (status IN ('pending', 'superseded', 'processed', 'failed'))
);
...
CREATE TABLE IF NOT EXISTS roster_move_request_players (
    ...
    UNIQUE (roster_move_request_id, player_mlb_id)
);
...
CREATE INDEX IF NOT EXISTS idx_team_player_mlb_id
    ON team_player(player_mlb_id);
CREATE INDEX IF NOT EXISTS idx_mlb_roster_changes_date
    ON mlb_roster_changes(change_date);
...
BEGIN
  ...
  VALUES (
    ...
    'players',
    'INSERT',
    json_build_object('mlb_id', NEW.mlb_id, 'name', NEW.name),
...
BEGIN
  ...
  VALUES (
    ...
    'team_player',
    'INSERT',
    json_build_object('id', NEW.id, 'team_id', NEW.team_id, 'player_mlb_id', NEW.player_mlb_id),
    (SELECT row_hash FROM audit ORDER BY id DESC LIMIT 1),
    encode(gen_random_bytes(16), 'hex')
  );
  RETURN NEW;
...
BEGIN
  ...
  VALUES (
    ...
    'INSERT',
    json_build_object('id', NEW.id, 'roster_move_request_id', NEW.roster_move_request_id, 'player_mlb_id', NEW.player_mlb_id, 'action', NEW.action, 'priority', NEW.priority),
    (SELECT row_hash FROM audit ORDER BY id DESC LIMIT 1),
    encode(gen_random_bytes(16), 'hex')
  );
  RETURN NEW;
...
CREATE OR REPLACE FUNCTION audit_insert_alumni() RETURNS trigger AS $$
BEGIN
  INSERT INTO audit (datetime, user_id, table_name, operation, new_value, prev_hash, row_hash)
  VALUES (
    CURRENT_TIMESTAMP,
    ...
    'alumni',
    'INSERT',
    json_build_object('id', NEW.id, 'player_mlb_id', NEW.player_mlb_id, 'team_id', NEW.team_id, 'deactivated_at', NEW.deactivated_at),
    (SELECT row_hash FROM audit ORDER BY id DESC LIMIT 1),
    encode(gen_random_bytes(16), 'hex')
  );
  RETURN NEW;
...
CREATE OR REPLACE FUNCTION audit_insert_mlb_roster_changes() RETURNS trigger AS $$
BEGIN
  INSERT INTO audit (datetime, user_id, table_name, operation, new_value, prev_hash, row_hash)
  VALUES (
    CURRENT_TIMESTAMP,
    ...
    json_build_object(
      'id', NEW.id,
      'player_mlb_id', NEW.player_mlb_id,
...
DROP TRIGGER IF EXISTS audit_mlb_roster_changes_ai ON mlb_roster_changes;
...
CREATE TRIGGER audit_mlb_roster_changes_ai
AFTER INSERT ON mlb_roster_changes
FOR EACH ROW EXECUTE FUNCTION audit_insert_mlb_roster_changes();
"""
